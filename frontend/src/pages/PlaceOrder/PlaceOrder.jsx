...
const PlaceOrder = () => {
  ...
  const initPay = async (order,amount) => {
    const options = {
      ...
      handler: async (response) => {
        try {
          const {data} = await axios.post(
            'http://localhost:4000/api/razor/verify-razor',
  ...
  const paymentRazorpay = async (planId,amount) => {
    try {
      if(!token) {
      ...
      const {data} = await axios.post(
         'http://localhost:4000/api/razor/pay-razor', 
        {planId,amount}, 
        {headers: {token}}
      )

      if(data.success) {
        initPay(data.order,amount)
      } else {
        console.log(data);
      }
    } catch (error) {
  ...
  const placeOrder = async (event) => {
    ...
    await paymentRazorpay(data.email,orderData.amount).then(async ()=>{
